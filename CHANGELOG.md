...
- **Queue watchdog** — BUG-01 fix: added `_queue_watchdog()` background thread to `Dispatcher` that scans the pending queue every 30s and notifies users when tasks have waited longer than `task_timeout`. `_expire_stale_pending_tasks()` drains stale tasks with Slack notification. Design: [openspec/changes/20260419-codex-rules-injection/](openspec/changes/20260419-codex-rules-injection/)
- **Timeout auto-retry** — BUG-02 fix: first timeout on attempt 0 now auto-retries once with cleared session (5s wait) before reporting failure. Changed timeout error message to Chinese: "任務執行超時（已自動重試一次仍失敗）...".
- **File path routing rules** — BUG-03 fix: added file path routing table to `environment.md` and `personality/prompt_hook.py` — Owner/Admin stores files in `workspace/files/{thread_ts}/` (persistent), regular users in `/tmp/opentree/{thread_ts}/` (ephemeral); both must upload via upload-tool.
...
### Fixed
- **AGENTS.md marker inconsistency** — `generate_agents_md()` in `generator/claude_md.py` now uses HTML-comment markers (`<!-- OPENTREE:AUTO:BEGIN/END -->`) matching the runtime `_write_agents_md()` in `codex_process.py`. Previously init used `# OPENTREE:AUTO:BEGIN` (markdown) while runtime used `<!-- ... -->`, causing each to treat the other's block as "owner content" and producing duplicate stale content on every deployment. Design: [openspec/changes/20260419-codex-rules-injection/](openspec/changes/20260419-codex-rules-injection/)
