...
- [x] ~~更新 DEPLOYMENT.md 加入 `opentree stop` 文件~~ ✅ (930082f)
- [x] ~~在 bot_walter 實例上實際驗證 `opentree stop`~~ ✅ 正常 stop 成功（60s timeout）、--force 成功
- [x] ~~bot_walter init --force 更新 run.sh~~ ✅ wrapper.pid 已確認寫入
- [x] ~~清理 legacy config/.env~~ ✅ 與 .env.local 內容相同，已移除
...
- [ ] 修正 --force SIGKILL 後 orphan bot 進程未被清理的問題（lifecycle.py 需追殺 bot.pid）
