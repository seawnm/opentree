...
72 tests (71 pass + 1 xfail), coverage 98%, 39 files created.
詳見 progress.md 第五節。
...
3. **Schema pattern**：`name` 用 `^[a-z][a-z0-9-]*$`，`version` 用 semver regex，`loading.rules` 用 `^[a-z0-9-]+\.md$`（純檔名，無 rules/ 前綴）
